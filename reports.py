...
def plot_confusion_matrix(cm,
    ...
    if target_names is not None:
        ...
        for i in range(0,len(target_names)-1):
            label_marks.append(alpha)
            alpha = chr(ord(alpha)+1)

        label_marks.append('Space')
        plt.xticks(tick_marks, label_marks, rotation=45)
        plt.yticks(tick_marks, label_marks)
...
model = load_model(cnf.KERAS_PATH)
pred_labels = []
...
plot_confusion_matrix(cm, range(27), normalize=False)
